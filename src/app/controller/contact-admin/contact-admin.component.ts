...
itemsPerPage: number = 5;
  ...
  getUserType(): string | null {
    ...
    if (this.userType === 'staff') {
      this.displayedColumns = ['select', 'fullname', 'phone','email', 'course', 'createdDateTime', 'leadSelection', 'followupDateTime', 'comments', 'Action','createdBy','delete'];
...
editFollowup(contact: any): void {
  ...
  dialogRef.afterClosed().subscribe(result => {
    if (result) {
      ...
      this.contactServices.updateContact(contact._id, updateData)
        .subscribe(
          response => {
            console.log('Follow-up updated successfully:', response);
            // Update the local data
            const index = this.dataSource.data.findIndex(item => item._id === contact._id);
            if (index !== -1) {
              this.dataSource.data[index].followup_date = updateData.followup_date;
              this.dataSource.data[index].followup_time = updateData.followup_time;
              this.dataSource._updateChangeSubscription();
            }
